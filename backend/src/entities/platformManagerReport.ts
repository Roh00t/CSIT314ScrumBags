...
import { ServiceBookingReportDatum } from "../dto/dataClasses";
...
export class ServiceBooking{
    ...
    public async generateDailyReport(myDate: Date): Promise<ServiceBookingReportDatum[]>{
        const dailyreport = await this.db.select(
        ...
        .leftJoin(servicesTable, 
            eq(serviceBookingsTable.serviceID, 
            servicesTable.id))
        .leftJoin(userAccountsTable, 
            eq(serviceBookingsTable.cleanerID, 
            userAccountsTable.id))
        .leftJoin(servicesProvidedTable,
            eq(serviceBookingsTable.cleanerID,
                servicesProvidedTable.cleanerID
            )
        )
        .where(and(gte(serviceBookingsTable.startTimestamp, 
            myDate), lt(serviceBookingsTable.startTimestamp, new Date(myDate.getDate() + 1))))
